...
int main(void){
    ...
    while(true){
        ...
        commandIO(cBuffer,circularCommandOutputBuffer,false);        // insert commands here into the inputBuffer and output there function output in the OutputBuffer
        if(commandEnteredBool()){
            ...
            set_sleep_mode(SLEEP_MODE_IDLE);  // sleep mode idle at the end to not interfere with the code execution.
            sleep_enable();
            sleep_mode();
        }
    }
